...
_(this was originally posted on [https://www.bekk.christmas/post/2020/12/functional-typescript-with-fp-ts](https://www.bekk.christmas/post/2020/12/functional-typescript-with-fp-ts))_
...
[^1] _These are not «real» type classes, they are type classes implementet with interfaces. You can’t use the same `map` function on all `Functors`, but all `Functors` has a `map` function. You still need to use the type specific implementation of `map`, but at least it encourages the same pattern._
